...
class axilite:
	...
	mem = None
	fd = -1
	def __init__(self,addr=0xA0000000,size=0x1000):
...
class axidma:
	...
	def __init__(self,axil_offset=0xA0000000,buf_size=system_buf_size,buf_offset=0,dma_buflen=26):
...
class axicdma:
	axil = None
	dma_bufsize = 2**23
	def __init__(self,axil_offset=0xA0000000):
		self.axil = axilite(axil_offset)
...
class plddr:
	...
	pl_base_addr = 0x400000000
	def __init__(self,cdma_offset=0xA0000000,ps_buf_offset=0,ps_buf_size=system_buf_size,pl_ddr_size=16*1024**3,pl_ddr_mapbase=0x400000000):
		...
		self.pl_base_addr = pl_ddr_mapbase
	def buf_write(self,raw_bytes,offset=0):
		return self.phy_buf.write(raw_bytes,offset)
	def buf_read(self,size,offset=0):
		return self.phy_buf.read(size,offset)
	def get(self,ps_offset=0,pl_offset=0,size=4096,sync=False):
		return self.cdma.movedata(self.pl_base_addr+pl_offset,self.ps_base_addr+ps_offset,size,sync)
	def put(self,ps_offset=0,pl_offset=0,size=4096,sync=False):
		return self.cdma.movedata(self.ps_base_addr+ps_offset,self.pl_base_addr+pl_offset,size,sync)
	def clean(self):
...
class easy_mm2s:
	axil = None
	aximm_mapbase = 0
	def __init__(self,axil_offset=0xA0000000,aximm_mapbase=0):
		self.axil = axilite(axil_offset)
		self.ddr_mapbase = aximm_mapbase
	def mm2s(self,size,addr=0,timeout=5000,sync=False):
		SA_ADDR = 0x0
		LEN_ADDR = 0x8
		START_ADDR = 0x10
		ERR_ADDR = 0x14
		READY_ADDR = 0x18
		self.axil.write32(0,START_ADDR)
		err = self.axil.read32(ERR_ADDR)
		if err != 0:
			print('EASY_AXI_MM2S has reported an error!')
			return 1
		for i in range(timeout):
			isIdle = self.axil.read32(READY_ADDR) #idle
			if isIdle == 1:
				break
			else:
				time.sleep(0.001)
			if i != timeout-1:
				print('EASY_AXI_MM2S is not ready yet!')
				return 1
		self.axil.write64(addr+self.ddr_mapbase,SA_ADDR)
		self.axil.write64(size,LEN_ADDR)
		self.axil.write32(1,START_ADDR)
		if sync:
			for i in range(timeout):
				isIdle = self.axil.read32(READY_ADDR) #idle
				if isIdle == 1:
