...
int main(int argc, char **argv) {
	u_int32_t endian_test = 1;
	//u_int32_t endian_test = 0;
	int fix_endian = 0;
	...
	//set up the filtering instruction	
	char filter_instr[1024];
	memset(filter_instr, '\0', sizeof(char)*1024 );
	for (int i = 1; i < argc; i++) {
		strcat(filter_instr, argv[i]);
		strcat(filter_instr, " ");
	}
	
	//printf("filtering instruction %s\n", filter_instr);
	
	if(pcap_compile_nopcap(65535, DLT_EN10MB, &fp, filter_instr, 1, PCAP_NETMASK_UNKNOWN) < 0) {
